...
export const sections = [
  { title: 'nexus-cli v0.10.0 - v0.16.6', id: 'nexus-cli-v0-10-0-v0-16-6' },
  { title: 'v0.13.x', id: 'v0-13-x' },
  { title: 'v0.12.0', id: 'v0-12-0' },
  { title: 'v0.11.x', id: 'v0-11-x' },
  { title: 'v0.10.3', id: 'v0-10-3' },
  { title: 'v0.10.1', id: 'v0-10-1' },
...
## nexus-cli v0.10.0 — v0.16.6

**Command-line tool release history.** The sections below group the nexus-cli changes since v0.10.0 by feature area rather than strict version order; see the [CLI reference](/cli) for full usage of each command. Current release: v0.16.6.

### `nexus run` lifecycle (v0.11.0 to v0.16.6)

- Pre-launch check suite: workspace state, auth, MCP config, plugin env vars, tool binary, and live Headroom preflight mode verification
- Launch countdown / confirmation prompt before exec-launching the target tool
- New flags: `--force` (skip countdown), `--skip-checks` (skip the check suite), `--exec` (replace process image instead of spawning a child)
- Post-session summary: git activity, token usage and cost, Nexus activity stats, and Headroom compression stats

### `nexus config` local/global precedence (v0.16.0)

- Git-style two-layer configuration: `--local` (project-scoped) and `--global` (machine-wide)
- Read-side precedence: local overrides global overrides built-in default
- `nexus config show` now displays provenance (which layer each value was read from)

### `nexus push` / `nexus stash` (v0.14.0 to v0.15.1)

- `nexus push`: bidirectional workspace sync via a fork-based publish model, with an origin guard requiring a prior `nexus pull`
- `nexus push --adopt-local`: first-time bootstrap path that adopts local state as the project baseline
- `nexus stash save|pop|list`: snapshot, restore, and list local workspace state around a `pull` or `push`

### `nexus project link/rotate/unlink/status` (v0.15.0)

- Project-scoped Nexus Model Gateway inference token (`nxs_proj_*`) issuance, zero-downtime rotation, and revocation
- Token status inspection independent of the general `nexus status` command

### Credential resolution and auto-sync (v0.14.1, v0.14.4, v0.16.6)

- Documented resolution order: CLI flag > environment variable > local config > global config > built-in default, for `NEXUS_API_URL`, `NEXUS_PRIVATE_TOKEN`, and `NEXUS_PROJECT_TOKEN`
- `nexus run` auto-syncs MCP credentials baked into `opencode.json` / `.claude/mcp.json` with the current login on every invocation

### `nexus status` / `nexus preflight` server-side validation (v0.16.0, v0.16.5)

- Real API-backed project existence and access checks, replacing local-cache-only checks
- Live-verified Headroom preflight that distinguishes an active `transform` mode from a silently-downgraded `observe` mode

### `nexus pull` / `nexus init` behavior changes (v0.16.1, v0.16.2)

- `export_warnings` gate: model-routing or provider configuration issues now block the export with an interactive confirm prompt, bypassable with `--yes` / `--force`
- `opencode_instructions` merged automatically into the generated `opencode.json`
- `NEXUS_PROJECT_ID` written into the MCP server environment block

### Security hardening (v0.13.1, v0.13.5, v0.16.3)

- `nexus shadow` git index safety fix (v0.13.1): guard against silently staging deletion of files with existing commit history
- Path traversal protection on sync manifest paths, automatic git-exclude of token-bearing config files, install-script checksum verification for `nexus upgrade`, allowlist sanitization of server-supplied preflight check commands, and reduced PAT token exposure in preflight/status output (v0.13.5)
- Clean error output (v0.16.3): raw Rust stack backtraces no longer leak on auth/API errors regardless of `RUST_BACKTRACE`

---

## v0.13.x — September 2026

**AI Gateway: BYOK, Model Catalog Rebuild, Routing Lab & Gateway Operations**

### v0.13.1 — BYOK: Provider Credentials & Project Billing Bindings (ADR-0096)

- Bring-your-own API key for OpenRouter, Anthropic, or OpenCode Zen under Settings → Provider Keys
- Keys encrypted at rest and verified against the provider's own key-info endpoint (no billable test request)
- Explicit, audited billing binding per project on `execution_mode = nexus_gateway`, replacing inference from the runtime-token creator
- New API surface: `POST /api/user-credentials`, `PATCH /api/projects/[projectId]/provider-binding`

### v0.13.2 — Model Catalog Rebuild (ADR-0097, ADR-0100)

- Catalog tab split into Registry (curated, routable) and Discover (full raw OpenRouter + models.dev feed)
- Nightly `pg_cron` sync job pulling OpenRouter and models.dev catalogs into Postgres
- OpenCode Zen added as a first-class aggregator alongside OpenRouter
- Catalog drift log recording upstream price/alias/context-length changes before they apply to active deployments

### v0.13.3 — Routing Lab: Evidence-Based Model Suitability (ADR-0104)

- Evidence layer combining LiveBench, LMArena, and Nexus scenario fixtures, each source weighted by trust/precedence
- Scenario-based recommendations with separately labeled quality/cost/latency lenses; missing evidence shown as missing, never faked
- Deterministic and LLM-judged fixture evaluations against configured deployments

### v0.13.4 — Gateway Operations

- Route Simulator: dry-run a routing profile/scenario against live gateway policy, with a live confirmation banner once observed in production
- Project-scoped route decision audit trail (selected deployment, fallback reason, budget state)
- Platform-admin cost reconciliation comparing provider account usage/credits against the internal cost ledger (`GET /api/admin/cost-reconciliation`)

### v0.13.5 — Security & Data-Integrity Hardening

- Consistent `apiError()` responses across the gateway API surface
- Atomic credential, skill, and evidence writes
- Outbound request timeouts and a cache-invalidation race fix
- Closed a project re-parenting authorization gap
- Fixed a compliance-jurisdiction bug for OpenRouter-routed vendor/model IDs
...
A large feature and security release. It introduces confidentiality-aware action gating, an admin audit log with SIEM export, per-user feature flags, and an alpha document-extraction pipeline.

### Feature: Confidentiality-Aware Action Gating (ADR-0079)

- Policy engine that evaluates sensitive actions against project confidentiality severity and returns one of three outcomes: `allow`, `warn_and_override`, or `block`
- Textract extraction is gated for medium-severity (C.2/C.3) projects
- Confidentiality override dialog with a permanent override audit trail
- New server-side evaluation endpoint: `POST /api/confidentiality-gate`

### Feature: Admin Audit Log + SIEM Export

- Admin Audit Log at `/dashboard/admin/audit-log` with full-text search, severity and flag filters, and an event detail view
- SIEM export via `GET /api/siem/events` with token authentication (`nxs_siem_*` prefix), returning NDJSON and CEF formats
- Admin download at `GET /api/admin/audit-events/export`
- SIEM token management: owner-only, one-time secret display, configurable expiry, and revoke

### Feature: Feature Flags (Alpha)

- Per-user, time-limited, revocable feature flags with an admin toggle UI
- Initial flags: `textract_extraction`, `knowledge_graph`, `llm_bridge`, `ci_bridge`
- Flags are an entitlement mechanism, not a general-availability switch

### Feature: AWS Textract Pipeline (Alpha)

- Asynchronous document text extraction, gated behind the confidentiality action-gating framework and the `textract_extraction` feature flag
- Per-document status badges reflecting extraction state

### Feature: Platform + User Settings

- Tenant management with name and slug editor, plus tabbed platform settings
- Editable user profiles and password change for email-auth accounts
- Landing-page tree picker navigation

### CodeRabbit Review Workflow

- New `coderabbit_review` ingest-document classification and `coderabbit_fix` task category
- Surfaced with badges, icons, and quick filters across the documents and tasks views

### Operational Tooling

- Three-file environment model: `.env.shared` + `.env.staging` + `.env.production`
- Deploy pipeline via Makefile, Sentry environment and release correlation, and S3 structured logging
- Workspace sync API: new `ws_push` and `af_status` MCP actions

### Security Posture

- Service-role access policy scoping for `user_feature_flags` and `audit_events`
- Fail-closed staging seed guard requiring an explicit target allowlist and HTTPS
- gitleaks fail-closed pre-commit hook
- CEF output escaping for SIEM export

<Note>
  For MCP tool additions in this release (`ws_push`, `af_status`), see the [MCP Tool Index](/mcp-tools). The authoritative tool count and version are published by `nexus-mcp`.
</Note>

---

## v0.11.x — July to August 2026

The 0.11 line focused on cross-project knowledge access, public product pages, observability, and workspace editing.

### v0.11.12 — August 14, 2026

- **WorkspaceIDE script editor** (ADR-0076)
- **Tabbed fork edit view** (ADR-0077)
- **Package table** with bidirectional JSON sync

### v0.11.11 — August 10, 2026

- **Personal address** (nickname / first name) with a `nickname` column on profiles
- **CURRENT USER block** injected into generated agent files

### v0.11.10 — August 8, 2026

- Patch release. See the nexus-hub `CHANGELOG.md` for the full bullet detail.

### v0.11.9 — August 5, 2026

- **Ingest document Source/Preview toggle**
- **`pr_draft` classification type** for ingest documents

### v0.11.8 — August 5, 2026

- **Dashboard ADR section** with ADR search and status filter

### v0.11.7 — July 29, 2026

- **Time investment widget** and time-stats API

### v0.11.6 — July 24, 2026

- **`/why-nexus` product story page** and a living design-system page

### v0.11.5 — July 21, 2026

- **ISR for public pages** and critical CSS inlining

### v0.11.4 — July 19, 2026

- **Sentry integration** and session timeline filter tabs

### v0.11.3 — July 17, 2026

- **FAQ / Why-Nexus page** and landing page

### v0.11.2 — July 16, 2026

- **Shared DataTable and CardGrid components**

### v0.11.1 — July 16, 2026

- **User UI preferences** and document versioning

### v0.11.0 — July 14, 2026

- **Cross-Project Knowledge Browser** and public API routes
