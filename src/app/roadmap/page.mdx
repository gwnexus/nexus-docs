...
- BYOK: Provider Credentials & Project Billing Bindings (ADR-0096) -- bring your own OpenRouter, Anthropic, or OpenCode Zen key, encrypted at rest and verified against the provider's key-info endpoint; explicit, audited billing binding per project (0.13.1)
- Model Catalog Rebuild (ADR-0097, ADR-0100): Registry/Discover split, nightly `pg_cron` sync from OpenRouter and models.dev, OpenCode Zen as a first-class aggregator, catalog drift log (0.13.2)
- Routing Lab (ADR-0104): evidence-based model suitability using LiveBench, LMArena, and Nexus scenario fixtures, with honestly-labeled quality/cost/latency lenses (0.13.3)
- Gateway Operations: Route Simulator dry-run with live confirmation, project-scoped route decision audit trail, platform-admin cost reconciliation against provider usage (0.13.4)
- Security & data-integrity hardening: consistent `apiError()` responses, atomic credential/skill/evidence writes, outbound timeouts, cache-invalidation race fix, project re-parenting authorization gap closed, OpenRouter compliance-jurisdiction fix (0.13.5)
- See [AI Gateway](/ai-gateway) for full documentation
